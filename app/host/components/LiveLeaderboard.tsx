...
export function LiveLeaderboard({ state, present, presenceReady }: { state: GameState; present: Set<string>; presenceReady: boolean }) {
  ...
  // Survival ranks by lives remaining (no points); other modes by score.
  const ranked = state.players.slice().sort((a, b) =>
    survival ? ((state.lives[b.id] ?? 0) - (state.lives[a.id] ?? 0)) || (b.score - a.score) : b.score - a.score)
          ...
          <div key={p.id} className="flex items-center gap-3 rounded-xl px-3 py-3 transition-all"
            ...
            <div className="flex-1 min-w-0">
              <div className="font-bold truncate text-2xl flex items-center gap-1.5">
                {p.name}
                {off && <span className="text-base" style={{ color: 'var(--muted)' }}>(away)</span>}
                {p.streak > 1 && <span className="text-base" style={{ color: 'var(--accent)' }}>streak {p.streak}</span>}
              </div>
              {survival && <div className="text-base mt-0.5">{lives > 0 ? <Hearts n={lives} /> : <span style={{ color: 'var(--incorrect)' }}>OUT</span>}</div>}
              {!survival && (state.phase === 'guessing' || state.phase === 'reveal') && (bet !== 1 || locked) && (
                <div className="text-base flex items-center gap-1.5 mt-0.5" style={{ color: 'var(--muted)' }}>
                  ...
                  {bet === 'shield' && <span style={{ color: '#60a5fa' }}>🛡 shield</span>}
